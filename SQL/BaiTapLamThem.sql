...
SELECT * FROM SANPHAM INNER JOIN DMSANPHAM ON DMSANPHAM.MaDM = SANPHAM.MaDM WHERE (SANPHAM.SoLuong > 50 AND DMSANPHAM.TenDM = N'Thoi trang nu') OR (SANPHAM.SoLuong > 100 AND DMSANPHAM.TenDM = N'Thoi trang nam')
...
SELECT SUM(SANPHAM.SoLuong) AS SoLuong, KHACHHANG.TenKH FROM KHACHHANG INNER JOIN DONHANG ON DONHANG.MaKH = KHACHHANG.MaKH INNER JOIN CHITIETDONHANG ON CHITIETDONHANG.MaDH = DONHANG.MaDH INNER JOIN SANPHAM ON SANPHAM.MaSP = CHITIETDONHANG.MaSP GROUP BY(TenKH)
...
SELECT KHACHHANG.MaKH , KHACHHANG.TenKH FROM KHACHHANG WHERE KHACHHANG.TenKH NOT IN (
SELECT KHACHHANG.TenKH
FROM KHACHHANG INNER JOIN DONHANG ON DONHANG.MaKH = KHACHHANG.MaKH INNER JOIN CHITIETDONHANG ON CHITIETDONHANG.MaDH = DONHANG.MaDH INNER JOIN SANPHAM ON SANPHAM.MaSP = CHITIETDONHANG.MaSP GROUP BY TenKH
)
...
SELECT KHACHHANG.MaKH FROM KHACHHANG INNER JOIN DONHANG ON DONHANG.MaKH = KHACHHANG.MaKH INNER JOIN CHITIETDONHANG ON CHITIETDONHANG.MaDH = DONHANG.MaDH INNER JOIN SANPHAM ON SANPHAM.MaSP = CHITIETDONHANG.MaSP INNER JOIN THANHTOAN ON THANHTOAN.MaTT = DONHANG.MaTT GROUP BY KHACHHANG.MaKH HAVING COUNT(KHACHHANG.MaKH) = 1
